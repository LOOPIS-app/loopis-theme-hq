 ...
 * 
 * Notice: The default WP login page is using login_head instead of wp_head and needs special treatment.
 * 
...
// Render a named slot from dedicated templates in templates/signup.
function loopis_theme_hq_login_template_slot($slot) {
    ...
    $template = get_stylesheet_directory() . '/templates/signup/' . $template_file;
...
function loopis_theme_hq_login_assets() {
    ...
    wp_enqueue_style('loopis-theme-hq-login', LOOPIS_THEME_HQ_URI . '/assets/css/login.css', array(), filemtime(LOOPIS_THEME_HQ_DIR . '/assets/css/login.css'));
}
