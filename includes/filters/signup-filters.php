...
/**
 * Render a signup template from templates/signup.
 */
...
function loopis_theme_hq_signup_assets() {
    wp_enqueue_style('loopis-theme-hq-signup', LOOPIS_THEME_HQ_URI . '/assets/css/signup.css', array(), filemtime(LOOPIS_THEME_HQ_DIR . '/assets/css/signup.css'));
}
...
function loopis_theme_hq_signup_extra_name_fields() {
    ...
    $postcode = isset($_POST['postcode']) ? loopis_theme_hq_normalize_postcode(wp_unslash($_POST['postcode'])) : '';
    ?>
    <p>
    ...
    <p>
        ...
        <input type="text" name="postcode" id="postcode" value="<?php echo esc_attr($postcode); ?>" autocomplete="postal-code" inputmode="numeric" pattern="[0-9]{5}" maxlength="5" required />
    </p>
    <p>
...
function loopis_theme_hq_signup_username_ui_bridge() {
    ...
    (function() {
        ...
        function moveNameFieldsToTop() {
            ...
            if (!form || !firstNameInput || !lastNameInput || !postcodeInput) {
                return;
            }

            var firstNameRow = firstNameInput.closest('p');
            var lastNameRow = lastNameInput.closest('p');
            var postcodeRow = postcodeInput.closest('p');
            ...
            if (!firstNameRow || !lastNameRow || !postcodeRow) {
                return;
            }
            ...
            if (lastNameRow.nextElementSibling !== postcodeRow) {
                form.insertBefore(postcodeRow, lastNameRow.nextElementSibling);
            }
        }
...
function loopis_theme_hq_validate_signup_name_fields($result) {
    ...
    if ('' === $postcode) {
        $result['errors']->add('postcode', __('Please enter your postcode.', 'loopis-theme-hq'));
    } elseif (!preg_match('/^[0-9]{5}$/', $postcode)) {
        $result['errors']->add('postcode', __('Postcode must be exactly 5 digits.', 'loopis-theme-hq'));
    }
...
/**
 * Output custom signup page header from isolated template, then open page-padding wrapper.
 */
...
/**
 * Close the page-padding wrapper opened before the signup form.
 */
function loopis_theme_hq_signup_close_page_padding() {
    echo '</div><!-- .page-padding -->';
}
