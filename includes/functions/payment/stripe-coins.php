...
function loopis_get_coins_user_id_from_session($session) {
    ...
    if (!empty($session['client_reference_id'])) {
        $client_reference_user_id = absint($session['client_reference_id']);
        if ($client_reference_user_id > 0 && get_userdata($client_reference_user_id)) {
            return $client_reference_user_id;
        }
    }
 ...
 * 
 * Webhook URL: https://<current-site>/wp-json/loopis/v1/stripe-coins-webhook
 * 
 * @param WP_REST_Request $request
 * @return WP_REST_Response
 */
function loopis_handle_stripe_coins_webhook($request) {
    error_log('LOOPIS: Stripe coins webhook received');
    ...
    $event_type = isset($event['type']) ? (string) $event['type'] : 'unknown';
    error_log("LOOPIS: Stripe coins webhook event type: {$event_type}");

    // Handle successful checkout events (sync and async settlement)
    if ($event_type === 'checkout.session.completed' || $event_type === 'checkout.session.async_payment_succeeded') {
        ...
        error_log("LOOPIS: Stripe coins webhook ignored event type: {$event_type}");
    }
...
function loopis_handle_coins_checkout_completed($session) {
    ...
    if (!loopis_is_coins_checkout_session($session)) {
        $metadata = isset($session['metadata']) && is_array($session['metadata']) ? $session['metadata'] : array();
        $checkout_type = isset($metadata['loopis_checkout_type']) ? sanitize_key((string) $metadata['loopis_checkout_type']) : '';
        $payment_link = isset($session['payment_link']) ? (string) $session['payment_link'] : '';
        error_log("LOOPIS: Coins checkout skipped because session did not match coins product (session {$session_id}, checkout_type {$checkout_type}, payment_link {$payment_link})");
        return;
    }
